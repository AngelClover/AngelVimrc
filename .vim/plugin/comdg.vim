...
" ToBeModify
  call append(line('$')-2, "#ifndef  __". s. "_")
  call append(line('$')-2, "#define  __". s. "_")
  call append(line('$')-2, "")
...
function! <SID>GetPyDocFileHeader(leading_blank)
    ...
    let doc = doc. a:leading_blank."\"\"\"\n"
    let doc = doc. a:leading_blank."File: ".<SID>GetFileName()."\n"
    let doc = doc. a:leading_blank."Author: "."AngelClover(AngelClover@aliyun.com)\n"
"<SID>GetUserName()."(".<SID>GetUserName()."@baidu.com)\n"
    let doc = doc. a:leading_blank."Date: ".<SID>GetDate()."\n"
    let doc = doc. a:leading_blank."\"\"\"\n"
    return doc
endfunction
...
map ;ltl    :call <SID>InsertFormat()<CR>
imap ;ltl <ESC>:call <SID>InsertFormat()<CR>

map ;lst    :call <SID>LatexLstListing()<CR>
imap ;lst <ESC>:call <SID>LatexLstListing()<CR>   

map ;item    :call <SID>LatexItemize()<CR>
imap ;item <ESC>:call <SID>LatexItemize()<CR>   

map ;frame    :call <SID>LatexFrame()<CR>
imap ;frame <ESC>:call <SID>LatexFrame()<CR>   

map ;sec    :call <SID>LatexSection()<CR>
imap ;sec <ESC>:call <SID>LatexSection()<CR>   

map ;doc    :call <SID>LatexDocument()<CR>
map ;beamer :call <SID>LatexBeamer2()<CR>
map ;enum   :call <SID>LatexEnumerate()<CR>
map ;graph  :call <SID>LatexGraph()<CR>

map ;para   :call <SID>LatexParagraph()<CR>
imap ;para   :call <SID>LatexParagraph()<CR>

map ;log :call <SID>BSLog()<CR>
map ;col    :call <SID>LatexColumns()<CR>
...
function! <SID>LatexLstListing()
  "call cursor(line('.'), col('$'))
  exec "normal! o\\begin{lstlisting}\r\\end{lstlisting}\<Esc>O"
  "call cursor(line('.')+2, col('$')-9)
endfunction
function! <SID>LatexGraph()
  "call cursor(line('.'), col('$'))
  exec "normal! o\\includegraphics[width=\\textwidth]{}\r\<Esc>k$i"
  "call cursor(line('.')+2, col('$')-9)
endfunction

function! <SID>LatexItemize()
  "call cursor(line('.'), col('$'))
  exec "normal! o\\begin{itemize}\r\\item\r\\item\r\\item\r\\end{itemize}\<Esc>kkkA"
  "call cursor(line('.')+2, col('$')-9)
endfunction

function! <SID>LatexEnumerate()
  "call cursor(line('.'), col('$'))
  exec "normal! o\\begin{enumerate}\r\\item\r\\item\r\\item\r\\end{enumerate}\<Esc>kkkA"
  "call cursor(line('.')+2, col('$')-9)
endfunction

function! <SID>LatexFrame()
  exec "normal! o\\begin{frame}\r\\frametitle{}\r\\end{frame}\<Esc>kf}i"
endfunction

function! <SID>LatexSection()
  exec "normal! o\\section{}\r\\subsection{}\r\\subsection{}\r\\subsection{}\<Esc>kkkf}i"
endfunction

function! <SID>LatexParagraph()
  exec "normal! o\\paragraph{}"
endfunction

function! <SID>LatexColumns()
    exec "normal! o\\begin{columns}\r\\begin{column}[c]{0.5\\textwidth}\r\\end{column}\r\\begin{column}[c]{0.5\\textwidth}\r\\end{column}\r\\end{columns}"
endfunction

function! <SID>LatexDocument()
    exec "normal! i\\documentclass{article}\r\\usepackage{CJK}\r\\usepackage{listings}\r\\usepackage{graphicx}\r\\usepackage{xcolor}\r\\lstset{language=bash,keywordstyle=\\color{blue!70}, commentstyle=\\color{red!50!green!50!blue!50}, frame=shadowbox, rulesepcolor=\\color{red!20!green!20!blue!20},escapeinside=``}\r\\begin{document}\r\\begin{CJK}{UTF8}{gbsn}\r\\title{}\r\\author{AngelClover}\r\\maketitle\r\r\\end{CJK}\r\\end{document}\r\<Esc>kkf}i"
endfunction

function! <SID>LatexBeamer2()
    exec "normal! i\\documentclass[transparency]{beamer}\r\\hypersetup{CJKbookmarks=true}\r%\\usepackage{ctex}\r\\usepackage{CJK}\r\\usepackage{xcolor}\r\\usepackage{listings}\r\\usepackage{graphicx}\r\\usetheme[secheader]{Madrid}\r\\usecolortheme{rose}\r\\begin{document}\r\\begin{CJK}{UTF8}{gbsn}\r\\AtBeginSection[]\r{\r\\begin{frame}\r\\tableofcontents[currentsection,hideallsubsections]\r\\end{frame}\r}\r\\AtBeginSubsection[]\r{\r\\begin{frame}[shrink]\r\\tableofcontents[sectionstyle=show/shaded,subsectionstyle=show/shaded/hide]\r\\end{frame}\r}\r\\pgfdeclareimage[width=3cm]{baidu}{baidu_logo.png}\r\\logo{\\pgfuseimage{baidu}{\\vspace{-10pt}}}\r\r\\end{CJK}\r\\end{document}\r\<Esc>kkf}i"
endfunction
function! <SID>LatexBeamer()
    exec "normal! i\\documentclass[transparency]{beamer}\r\\hypersetup{CJKbookmarks=true}\r\\%usepackage{ctex}\r\\usepackage{CJK}\r\\usepackage{xcolor}\r\\usepackage{listings}\r\\usepackage{graphicx}\r\\usetheme[secheader]{Madrid}\r\\usecolortheme{rose}\r\\begin{document}\r\\begin{CJK}{UTF8}{gbsn}\r\r\\end{CJK}\r\\end{document}\r\<Esc>kkf}i"
endfunction

function! <SID>BSLog()
    exec "normal! iBASA_BS_LOG(UL_LOG_WARNING, \"\");\r\<Esc>kf\"a"
endfunction
